# Version: CSF-CUF hollow-rectangle Bredt-Batho torsion post-processing v7 - 2026-08-28
...
def _rectangle_perimeter(bounds, points_per_edge: int = 41):
    """Return an ordered, closed sampling of one rectangular boundary."""
    n = max(3, int(points_per_edge))
    y_min = float(bounds["y_min"])
    y_max = float(bounds["y_max"])
    z_min = float(bounds["z_min"])
    z_max = float(bounds["z_max"])

    bottom_y = np.linspace(y_min, y_max, n, endpoint=False)
    right_z = np.linspace(z_min, z_max, n, endpoint=False)
    top_y = np.linspace(y_max, y_min, n, endpoint=False)
    left_z = np.linspace(z_max, z_min, n, endpoint=False)
...
def _write_deformed_section_plot(
    *,
    u,
    x: float,
    x_fraction: float,
    ...
    """Write an amplified transverse deformation plot from solved u(x,y,z)."""
    try:
        import matplotlib
        ...
        raise RuntimeError(
            "matplotlib is required to write the torsion deformation image"
        ) from exc

    u_at_x = _section_evaluator(u, x)
    contours = []
    maximum_transverse_displacement = 0.0

    for label, bounds in (
        ("Outer boundary", reference["outer"]),
        ("Hole boundary", reference["inner"]),
    ):
        y, z = _rectangle_perimeter(bounds)
        values = np.asarray(
            [
                _displacement(
                    u,
        ...
        uy = values[:, 1]
        uz = values[:, 2]
        transverse_norm = np.hypot(uy, uz)
        maximum_transverse_displacement = max(
            maximum_transverse_displacement,
            float(np.max(transverse_norm)),
        )
        contours.append((label, y, z, uy, uz))

    section_size = max(float(reference["B"]), float(reference["H"]))
    target_display_displacement = 0.18 * section_size
    if maximum_transverse_displacement > 0.0:
        amplification = target_display_displacement / maximum_transverse_displacement
    else:
        amplification = 1.0
    ...
    for color, (label, y, z, uy, uz) in zip(colors, contours):
        y_deformed = y + amplification * uy
        z_deformed = z + amplification * uz
        axis.plot(
            y,
            z,
            linestyle="--",
        ...
        axis.plot(
            ...
            linewidth=2.3,
            color=color,
            label=f"{label} – deformed",
        )

        arrow_step = max(1, (len(y) - 1) // 16)
        indices = np.arange(0, len(y) - 1, arrow_step)
        axis.quiver(
            y[indices],
            z[indices],
            amplification * uy[indices],
            amplification * uz[indices],
            angles="xy",
            scale_units="xy",
            scale=1.0,
            width=0.003,
    ...
    axis.plot([], [], "--", color="#777777", label="Undeformed boundaries")
    axis.axhline(0.0, color="#BBBBBB", linewidth=0.7, zorder=0)
    axis.axvline(0.0, color="#BBBBBB", linewidth=0.7, zorder=0)
    axis.set_aspect("equal", adjustable="box")
    axis.set_xlabel("y [mm]")
    axis.set_ylabel("z [mm]")
    axis.set_title(
        "Hollow section under torsion\n"
        f"x/L = {float(x_fraction):.3f}   "
        f"theta = {field_check['theta_y_pair']:.4e} rad"
    )
    axis.grid(True, linestyle=":", linewidth=0.7, alpha=0.55)
    axis.legend(loc="best")

    annotation = (
        f"Display amplification: {amplification:.4g}x\n"
        f"max transverse |u|: {maximum_transverse_displacement:.4e} mm\n"
        f"u_y/u_z twist mismatch: {field_check['mismatch_percent']:.4e}%"
    )
    axis.text(
        0.02,
        0.02,
    ...
    path = output_dir / "torsion_deformed_section_x050.png"
    figure.savefig(path, dpi=200)
    plt.close(figure)
    return path, float(amplification), float(maximum_transverse_displacement)
...
def write_outputs(u, model_bridge, case, problem_definition):
    ...
    image_path, image_amplification, image_max_u = _write_deformed_section_plot(
        u=u,
        x=x_mid,
        x_fraction=0.5,
        reference=reference,
        field_check=mid_check,
        output_dir=output_dir,
    )

    lines = [
        "# Version: CSF-CUF hollow-rectangle torsion analytical report v7 - 2026-08-28",
        ...
        "",
        "DEFORMED-SECTION IMAGE",
        "----------------------",
        f"file                          = {image_path.name}",
        f"display amplification         = {image_amplification:.12e}",
        f"maximum transverse |u| [mm]  = {image_max_u:.12e}",
        "The amplification affects only the drawing, not the reported values.",
        "",
