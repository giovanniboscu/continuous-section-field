...
The agreement between the two models also depends on the local three-dimensional effects generated by the boundary conditions. Near constrained boundaries, the displacement field may exhibit localized gradients and distortions that are more difficult to represent with the specific expansion adopted in this example. Consequently, a residual difference with the FEM3D solution may persist even when the response has become essentially independent of \(N\).
