...
Both problems use the same T-shaped CSF model defined and inspected in Step 1. The difference is the way in which the structure is loaded.

#### Bending half-wave problem

The bending problem is defined in:
...
The first block identifies the physical model:
...
Geometry and material properties are therefore not defined again here: the problem uses the same CSF model already inspected in Step 1.
...
This selects the already implemented surface half-wave loading law.

The loaded physical surface is identified directly from the CSF geometry:
...
In this case, the load acts on the edge of the `web` polygon that starts from vertex `0`. These identifiers are the same polygon and vertex identifiers shown during the geometry inspection in Step 1.
...
The load is a distributed traction acting in the global `z` direction, with a sinusoidal half-wave variation along the beam:

```text
amplitude * sin(pi * (x - x0) / L)
```

The sign of `amplitude` therefore defines the global `z` direction of the load.

The load is applied to the real physical surface generated by the selected CSF edge. The current implementation supports edges that are horizontal in the transverse section, although their global `z` position may vary along the beam.

The problem also includes the boundary conditions required by this predefined loading scheme: the global `y` and `z` CUF amplitudes are constrained at both beam ends, while the remaining rigid translation in global `x` is removed by fixing the physical axial displacement at the start section.

#### Torsion half-wave problem

The torsional problem is defined in:
...
Again, the same CSF model is used.

The selected problem type is:
...
This predefined problem applies two opposite longitudinally distributed loads in the global `z` direction. Their intensities vary with the same half-wave law:
...
The two load trajectories are determined automatically from the physical CSF geometry: one follows a vertex on the upper boundary and the other a vertex on the lower boundary. Their opposite signs generate the torsional action.

Unlike the bending problem, no surface selector is required in the YAML because the two physical load points are selected automatically by the predefined torsion adapter.

The torsional problem uses the same end constraints described above.

These two problems are only examples of the loading schemes already available in the current CSF-CUF implementation. Other predefined load types are also provided, and new problem or loading adapters can be added without modifying the CUF solver core.

The structural problem files therefore define **what is applied to the physical CSF model**, while the CUF case defined in Step 2 specifies **how that problem is approximated and solved**.
