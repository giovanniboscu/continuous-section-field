# Version: carrera_post_v2 - 2026-09-24
# OPT-04 SHARED FIXED-X POST CACHE
...
class _SectionEvaluationCache:
    """Cache the full three-component paper displacement at one fixed x."""

    def __init__(self, *, u, x: float, u_at_x=None):
        self.u = u
        self.x = float(x)
        self.u_at_x = u_at_x
        self._values = {}

    def vector(self, y: float, z: float) -> np.ndarray:
        y = float(y)
        z = float(z)
        key = (y, z)
        ...
        if self.u_at_x is None:
            value = _paper_displacement(self.u, self.x, y, z)
        else:
            value = _paper_displacement_fixed_x(self.u_at_x, y, z)

        self._values[key] = value
        return value


def _prepare_section_domains(section_provider, x: float, grid_n: int):
    """Build section sampling geometry once and reuse it for all components."""
    x = float(x)
    ...
    for domain in section_provider.domains(x):
        y_min, y_max, z_min, z_max = _domain_bounds(domain)
        is_rectangle = _is_axis_aligned_rectangle(domain)
        ...
        points = []
        if is_rectangle:
            # Every tensor-grid point lies in an axis-aligned rectangle, so the
            # generic point-in-polygon test would only repeat work.
            for y in ys:
                for z in zs:
                    points.append((float(y), float(z)))
        else:
            for y in ys:
                for z in zs:
        ...
        # Keep the original behaviour exactly: vertices are appended even when
        # they duplicate grid corners.
...
def section_displacement_extremum(
    *,
    ...
    if _evaluation_cache is None:
        _evaluation_cache = _SectionEvaluationCache(
            u=u,
            x=x,
            u_at_x=u_at_x,
        )

    if _prepared_domains is None:
        _prepared_domains = _prepare_section_domains(
            section_provider,
            x,
            grid_n,
        )
    ...
    def signed_value(y, z):
        return float(_evaluation_cache.vector(y, z)[component_index])

    for prepared in _prepared_domains:
        y_min, y_max, z_min, z_max = prepared["bounds"]
        points = prepared["points"]

        samples = [
            (signed_value(y, z), y, z)
            for y, z in points
        ]
...
def _report_spec(case, problem_definition, model_bridge, u):
    ...
    if problem_type in (
        ...
        "surface_halfwave_transverse_supported",
    ):
        factor = (
            (math.pi ** 4) / 12.0
            * (a ** 3) / (length ** 4)
...
def _section_extrema(u, model_bridge, x: float, spec):
    ...
    # Same fixed-x idea used by the lightweight post-processor, but the full
    # Carrera extrema search is preserved. Geometry and vector field values are
    # shared by ux, uy and uz instead of being rebuilt three times.
    evaluation_cache = _SectionEvaluationCache(
        u=u,
        x=x,
        u_at_x=u_at_x,
    )
    prepared_domains = _prepare_section_domains(
        model_bridge.section_provider,
        x,
        25,
    )
    ...
    for component in ("ux", "uy", "uz"):
        ...
        row = section_displacement_extremum(
            ...
            _prepared_domains=prepared_domains,
        )
        row["scaled_value"] = (
...
def _global_extrema(u, model_bridge, case, spec, section_cache=None):
    ...
    for x in xs:
        x = float(x)
        section = _section_extrema(u, model_bridge, x, spec)
        if section_cache is not None:
            section_cache[x] = section
