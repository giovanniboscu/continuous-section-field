# Version: CSF-CUF physical-surface half-wave paper-2010 adapter v3 - 2026-09-25
"""
Physical-surface half-wave load with Carrera/Navier-type end constraints.

Load:
    t_global(x) = (0, 0, amplitude * sin(pi * (x - x0) / L))

The traction is applied on the selected real physical CSF surface and uses the
real-surface measure, exactly as in ``surface_halfwave.py``.

Constraints (Carrera & Giunta 2010 / Navier form, mapped to CSF-CUF axes):
- at S0, global y and z displacement fields are zero while global x is free;
- at S1, global y and z displacement fields are zero while global x is free;
- no axial point anchor is imposed; the same mean axial gauge used by carrera_problem.py is imposed.

YAML::

    problem:
      type: surface_halfwave_transverse_supported
      surface:
        polygon_name: bottom_flange
...
PROBLEM_TYPE = "surface_halfwave_transverse_supported"
...
class SurfaceHalfWaveTransverseSupportedProblem:
    """Physical-surface half-wave load with transverse end restraints."""
    ...
    def build_constraints(
        ...
        *,
        ...
        """Use the same transverse supports and axial gauge as carrera_problem.py."""
...
def build_problem(problem_type: str, options: dict):
    ...
    return SurfaceHalfWaveTransverseSupportedProblem(
...
__all__ = (
    ...
    "SurfaceHalfWaveTransverseSupportedProblem",
    "build_problem",
)
