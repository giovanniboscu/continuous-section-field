...
def _run_case(executable: str, entry: CaseEntry, index: int, total: int) -> bool:
    print()
    print(
    ...
    if not entry.result_path.is_file():
        print(
            f"ERROR: solver finished but output was not found: {entry.result_path}",
            file=sys.stderr,
        )
        return False
...
def main() -> int:
    ...
    for index, entry in enumerate(all_cases, start=1):
        if _run_case(executable, entry, index, len(all_cases)):
