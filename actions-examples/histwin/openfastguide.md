...
This workflow sets up a **time-domain structural dynamics simulation** of a wind turbine tower using OpenFAST and ElastoDyn. The tower geometry is provided by a continuous description, from which the distributed mass and stiffness properties along the height are generated automatically. OpenFAST then uses those properties to compute the structural response.
...
This guide starts from an existing  CSF YAML tower geometric model, for example:
...
In the common manual FAST/NREL workflow, the step between BModes output and ElastoDyn input was often performed manually using the ModeShapePolyFitting.xls: the user pastes BModes modal data into the sheet, reads back the polynomial coefficients, and copies them into the ElastoDyn tower .dat file.
...
which automates the polynomial fit from the command line and adds two screening steps: automatic mode identification with twist-ratio filtering, to prefer bending-dominated modes over mixed or torsional ones, and tip-displacement threshold filtering, which rejects candidates whose tip displacement in the relevant axis is less than 1% of the maximum tip value among the candidates for that axis, reducing the risk of ill-conditioned fits.
...
of the distributed property table fed to BModes.

For a simple monotonic tower - where diameter and thickness vary smoothly from
