...
This guide explains how to use the CSF → BModes → ElastoDyn → OpenFAST pipeline starting from an already available CSF geometry file such as `histwin_tower.yaml`.

Its purpose is to show, in a clear operational way:

- which prerequisites must be installed,
- which files are used at each stage,
- which scripts must be executed and in which order,
- how to run the pipeline in both automatic and manual modes,
...
They are not intended to represent a real turbine configuration. All non-CSF machine data remain the user’s responsibility and must be replaced with physically consistent values for any meaningful engineering analysis.
