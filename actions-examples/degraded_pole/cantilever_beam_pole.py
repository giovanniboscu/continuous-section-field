...
def snap_small(
    ...
    """Snap numerical noise to zero using a dimensional reference scale."""

    value = float(value)
    scale = max(abs(float(scale)), 1.0)

    if abs(value) <= max(abs_tol, rel_tol * scale):
        return 0.0

    return value


def snap_actions_for_stress(
    section_field,
    actions: InternalActions,
) -> InternalActions:
    """Return the action vector used by stress APIs with numerical noise removed."""

    _, _, L = field_bounds(section_field)

    force_scale = max(abs(actions.N), abs(actions.Pp), 1.0)
    moment_scale = force_scale * max(abs(L), 1.0)

    return replace(
        actions,
        Mx=snap_small(actions.Mx, moment_scale),
        My=snap_small(actions.My, moment_scale),
        Tx_jourawski=snap_small(actions.Tx_jourawski, force_scale),
        Ty_jourawski=snap_small(actions.Ty_jourawski, force_scale),
        Tz=snap_small(actions.Tz, moment_scale),
        dMx_dz=snap_small(actions.dMx_dz, force_scale),
        dMy_dz=snap_small(actions.dMy_dz, force_scale),
    )
...
def is_s_polygon_name(name: Any) -> bool:
    """Return true when the CSF polygon name ends with the material tag ``S``."""

    base_name = strip_csf_name(name)
...
def _finite_shear_candidates(
    ...
    *,
    ...
    """Return finite governing-shear candidates for the requested polygon group."""
    ...
    for row in shear_rows:
        ...
        if is_s_polygon_name(polygon_name) != bool(select_s):
            continue
        ...
        for key in keys:
            ...
            direction = "x" if key.startswith("tau_x_") else "y"
            x_key = f"x_{key}"
            y_key = f"y_{key}"
...
def shear_extreme_rows_at_station(
    ...
    *,
    ...
    """Return all polygons sharing the largest ``abs(tau_governing)`` in one group."""
    ...
    max_abs_tau = max(abs(float(candidate["tau_governing"])) for candidate in candidates)
    tolerance = max(max_abs_tau, 1.0) * 1.0e-12

    rows = [
        candidate
        for candidate in candidates
        if abs(abs(float(candidate["tau_governing"])) - max_abs_tau) <= tolerance
    ]
    rows.sort(
        key=lambda row: (
            int(row["polygon_idx"]),
...
def write_mechanical_report(
    ...
    shear_extreme_rows_s: list[dict[str, Any]],
    shear_extreme_rows_non_s: list[dict[str, Any]],
) -> Path:
    ...
    widths = [11, 14, 14, 14, 14, 14, 14, 14, 14, 14]
    ...
    lines.append("")
    lines.append("SHEAR LEGEND")
    lines.append("------------")
    lines.append(
        "tau_governing : signed Jourawski shear-stress value having the largest"
    )
    lines.append(
        "                  absolute magnitude, selected among tau_x_min, tau_x_max,"
    )
    lines.append(
        "                  tau_y_min and tau_y_max. The original sign is preserved."
    )
    lines.append(
        "direction     : x when tau_governing is selected from tau_x_min or tau_x_max;"
    )
    lines.append(
        "                  y when selected from tau_y_min or tau_y_max."
    )
    lines.append("")
    ...
    shear_widths = [11, 18, 10, 12, 22, 14, 14]

    def append_shear_table(title: str, rows: list[dict[str, Any]]) -> None:
        lines.append(title)
        ...
        for row in rows:
            lines.append(
                ...
                f"{float(row['tau_governing']):{shear_widths[1]}.5e} "
                f"{str(row['direction']):>{shear_widths[2]}} "
    ...
    append_shear_table("SHEAR EXTREME ROWS - S", shear_extreme_rows_s)
    append_shear_table("SHEAR EXTREME ROWS - NON-S", shear_extreme_rows_non_s)
...
def run(settings: AnalysisSettings) -> list[Path]:
    ...
    station_summary_rows: list[dict[str, Any]] = []
    shear_extreme_rows_s_all: list[dict[str, Any]] = []
    shear_extreme_rows_non_s_all: list[dict[str, Any]] = []
    ...
    for i,z in enumerate(settings.z_stations, start=1):
        ...
        shear_extreme_rows_s_all.extend(
            shear_extreme_rows_at_station(shear_rows, select_s=True)
        )
        shear_extreme_rows_non_s_all.extend(
            shear_extreme_rows_at_station(shear_rows, select_s=False)
        )
            ...
            section_debug_rows.append(
                {
                    ...
                    "ex": prestress.ex,
                    "ey": prestress.ey,
                    "Mx_ext": raw_actions.Mx_ext,
                    "My_ext": raw_actions.My_ext,
                    "Mx_prestress": raw_actions.Mx_prestress,
                    "My_prestress": raw_actions.My_prestress,
                    "dMx_dz": raw_actions.dMx_dz,
                    "dMy_dz": raw_actions.dMy_dz,
                }
            )
    ...
    written_files.append(
        write_mechanical_report(
            ...
            shear_extreme_rows_s=shear_extreme_rows_s_all,
            shear_extreme_rows_non_s=shear_extreme_rows_non_s_all,
        )
    )
