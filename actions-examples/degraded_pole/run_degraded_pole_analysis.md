...
Before running the mechanical analysis, the CSF model can be inspected graphically with `csf-actions`.
...
This first step allows the geometry and the material-participation fields to be checked graphically before the mechanical analysis is started.

The files generated by these actions are written to:

```text
out/iso
```

After verifying the geometry and material fields, run the sectional mechanical analysis described below.
...
## 4. Files produced by the analysis
...
## 6. Important execution directory
