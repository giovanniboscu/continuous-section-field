...
In summary, these longitudinal variations define prescribed degradation fields within the CSF model. The user can modify them without changing the geometric definition of the pole. 
The generated field can then be evaluated at any height to inspect the local section, compute the corresponding sectional properties, or sample station-wise sections for downstream structural analysis. The same sectional evaluation is also required to recompute the eccentricity of the prestressing resultant when degradation of the prestressing-steel components is prescribed.
