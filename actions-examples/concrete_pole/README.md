...
In this case, the term `contribution` should be read as **elastic modulus contribution**.

The reason is that each concrete region and each prestressing bar is assigned a multiplier associated with that named zone. In this generated case, that multiplier represents the elastic modulus assigned to the zone, relative to the concrete reference value.
...
means the reference elastic modulus.
...
* base elastic-modulus multipliers assigned by the generator;
* lookup tables that modify the normalized elastic modulus along the pole.
...
Here, `pcbar_15` is the name of one prestressing bar.

The `vertices` list contains the polygon coordinates used to represent that bar in the section.
...
This means that the prestressing bars are assigned a base elastic modulus six times the concrete reference elastic modulus.
...
The `weight_laws` block is used for the elastic modulus that contributes to axial and bending stiffness.

The `shear_weight_laws` block is used for the elastic modulus that contributes to shear and torsion stiffness.
...
In this case, the axial/bending elastic modulus and the shear/torsion elastic modulus can follow different normalized laws.
...
means that the normalized elastic-modulus values are read from an external lookup table.
...
The same bar can also receive a separate shear/torsion elastic-modulus law:
