# API Reference - `plot_weight.py`

This document covers the top-level classes and functions defined in `src/csf/actions/plot_weight.py`. Imported symbols are not documented as standalone APIs here.

## Module summary

- Source file: `src/csf/actions/plot_weight.py`
- Output file: `docs/API/actions_plot_weight_api_en.md`
...
- `def registerregister_action: Any, *, ActionSpec: Any, ParamSpec: Any, Visualizer: Any` - line 298
...
`params.get`, `action.get`, `Visualizer`, `set`, `sorted`, `RuntimeError`, `int`, `plt.get_fignums`, `plt.figure`, `fig.set_label`, `matplotlib.rc_context`, `viz.plot_weight`, `join`, `cleaned.strip`, `getattr`, `Path`, `hasattr`, `strip`, `line.strip`, `len`, `outp.parent.exists`, `list`, `fig.canvas.draw`, `fig.canvas.get_renderer`, `plt.close`, `plt.gcf`, `ylabel.splitlines`, `startswith`, `_sanitize_filename_fragment`, `expanded`, `bbox.transformed`, `_axis_polygon_tags`, `outp.with_name`, `used_target_paths.add`, `fig.savefig`, `text.strip`, `label.startswith`, `split`, `ax.get_ylabel`, `fig.dpi_scale_trans.inverted`, `str`, `ch.isalnum`, `ax.get_tightbbox`, `line.get_label`

## Explicit registration hook (no side effects)

### `register`

**Source lines:** `298-319`
