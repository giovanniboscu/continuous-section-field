...
**Purpose** – CSF provides a modelling infrastructure in which the contribution of each cross-sectional component to the axial–bending and shear–torsional response is governed by two independent variation laws, queryable along the member axis and decoupled both from the geometry and from each other.
...
Because geometry and participation remain explicit functions of \(z\), mathematical operations can be applied directly to the generating field and to its derived quantities. The model may therefore be evaluated, composed, integrated and, where the prescribed laws are sufficiently regular, differentiated without reconstructing a continuous representation from sampled sectional data.

Sectional properties, solver inputs, and local stress recovery are obtained as different evaluations of the same \(S(z)\). Axial stations, sectional discretisations, and solver-specific representations are introduced only when required by a particular analysis and do not replace the field as the definition of the member.

To the authors’ knowledge, existing approaches for non-prismatic or graded members do not provide a general many-region formulation in which geometry, component identity, and material participation remain independently parametrised and persist as the primary representation throughout the computational workflow.
...
Longitudinal structural members are commonly described through local sections evaluated at selected axial coordinates. For members whose geometry and material participation vary independently along the axis, however, these local descriptions are only evaluations of a more general generating model. Reducing that model to assembled sections or sampled sectional quantities weakens the explicit relation between the laws governing the different components of the member.

Continuous Section Field (CSF) preserves this relation by defining the member through a field (S(z)). At each axial coordinate, the field returns an ordered collection of persistent cross-sectional regions, each characterized by its geometry, identity, and axial/bending and shear/torsion participation. The geometry and participation of each region may evolve independently along (z), while remaining coupled within the same representation of the member.

Because the generating laws remain explicit, (S(z)) can be evaluated, composed, integrated and, where the prescribed laws are sufficiently regular, differentiated. Local sections, sectional properties, solver quantities, and local mechanical states are obtained by applying different operators to the same field representation.

CSF therefore introduces a general and operational field formulation, not currently available in existing approaches, for representing the complete internal constitution of a longitudinal member through an arbitrary number of independently parametrised geometric and material regions. The contribution of this work is the formal definition of this many-region field, its composition rules, and its use as the common generating model throughout the computational workflow.
