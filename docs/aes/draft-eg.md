...
**Purpose** – CSF provides a component-based modelling methodology in which each section component acts as the geometric carrier of two independent longitudinal fields governing its axial–bending and shear–torsional participation. The resulting composite fields are queryable along the member axis and are decoupled both from the component geometry, including its longitudinal variation, and from each other.
