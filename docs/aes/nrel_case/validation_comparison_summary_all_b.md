...
This reference procedure does not read the YAML input files used by the beam model. It does not use OpenSees and does not call CSF section-sampling APIs. The required tower geometry, loading data, and material cases are defined directly in the reference script, and the response is evaluated by direct integration.
...
Path 2:  fixed tower data → independent continuous section-property functions → tolerance-based integration grid → analytical integration → reference response
```
...
This behaviour supports the main validation message. In the CSF-to-OpenSees path, the YAML file defines the sectional model as a continuous field. The OpenSees beam model receives a sampled representation of that field, while the independent continuous baseline provides a separate reference path based on the same tower data and loading assumptions.

The comparison therefore supports the consistency of:

* the YAML-defined continuous sectional field used by the CSF-to-OpenSees path;
* the OpenSees sampling of the continuous CSF field;
* the degraded stiffness law;
* the agreement with the independent continuous-baseline calculation.
