...
MODEL_STATION_SCENARIOS: List[Dict[str, Any]] = [
    {"label": "uniform 4", "n_elems": 4},
    {"label": "uniform 6", "n_elems": 6},
    {"label": "uniform 8", "n_elems": 8},
    {"label": "uniform 12", "n_elems": 12},
    {"label": "uniform 16", "n_elems": 16},
    {"label": "uniform 24", "n_elems": 24},
    {"label": "uniform 32", "n_elems": 32},
]

MODEL_IDS = [f"model_{i}" for i in range(len(MODEL_STATION_SCENARIOS))]

# Default internal section integration rule. Can be overridden with --gauss-points.
DEFAULT_N_GAUSS_POINTS = 2
...
parser.add_argument(
    "--gauss-points",
    type=int,
    default=DEFAULT_N_GAUSS_POINTS,
    help="Number of Gauss integration points per element. Default: 2.",
)
...
N_GAUSS_POINTS = int(ARGS.gauss_points)
OUTPUT_SUFFIX = "" if N_GAUSS_POINTS == DEFAULT_N_GAUSS_POINTS else f"_gauss{N_GAUSS_POINTS}"
OUTPUT_DIR = Path(f"openseeslab_output_{YAML_STEM}{OUTPUT_SUFFIX}")


def load_yaml_data(path: str) -> Dict[str, Any]:
    """Load the raw CSF YAML data used to build the OpenSees model."""
...
def print_csf_opensees_input_summary(yaml_file: str, gauss_points: int) -> None:
    """Print the CSF input data used by the CSF-OpenSees projection."""
    ...
    lines.append("-" * 78)
    lines.append("Section integration rule   : Gauss")
...
def run_segmented_model(
    ...
    """Run one segmented OpenSees model with N-point Gauss section sampling."""
    try:
    ...
    #   - N_GAUSS_POINTS internal Gauss-section calls for each element.
    ...
    for i in range(n_elems):
        ...
        sec_tags = [
            int(trunk_sections[n_ip * i + k].tag)
            for k in range(n_ip)
        ]

        # Same station mesh; Gauss section samples per element.
        # UserDefined receives: section tags, locations, weights.
        ops.beamIntegration(
            "UserDefined",
            int_tag,
            n_ip,
            *sec_tags,
            *[float(x) for x in gauss_xi],
            *[float(w) for w in gauss_w],
        )
...
def run_lab() -> Tuple[List[ModelResult], Dict[str, Dict[str, np.ndarray | float | str]], Path, Path, Path]:
    ...
    print_csf_opensees_input_summary(YAML_FILE, N_GAUSS_POINTS)
