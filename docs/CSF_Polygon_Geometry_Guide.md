...
The bridge is a straight segment connecting the end of one loop to the start of the other.  
It has no thickness, no area, and no geometric constraint beyond being a valid vertex sequence.  
There is no requirement to return to the bridge start point; the bridge is simply the transition between loops.
...
@cell and @wall Tags are metadata only.
...
> **Note:** The polygon name at `S1` must match the base name at `S0` for index-based pairing.  
> Tags in the name at `S1` are optional - pairing is always by list index, not by name.
...
> **Note:** Each strip is an independent polygon. CSF sums their torsional contributions.  
> Polygon pairing between `S0` and `S1` is index-based - the order in the list must be consistent.
...
- `weight: 1.0` is the standard value for material polygons.
