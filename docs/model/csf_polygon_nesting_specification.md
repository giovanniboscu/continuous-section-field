...
Root polygons (no container) have `W_eff_child = W_abs_child`.
...
```
At z = 10:  W_abs_parent = 30000 * exp(-0.5) ≈ 18 197
...
Partial overlaps are **not automatically detected**. The user is responsible for ensuring
correct section topology by inspecting **CSF's graphical output** before running any analysis.
...
defined first takes precedence.

```
⚠️  WARNING: Polygons 'web' and 'flange' are perfectly overlapping.
    Priority given to 'web' (defined first).
    Verify your section geometry.
```

> In normal nesting (full containment), creation order does not affect the result.
> Order matters only for perfect overlaps.
