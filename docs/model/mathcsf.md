...
# Straight-Axis Continuous Section Field
...
be the axial interval of a straight member, parameterized by the coordinate $z$.

A **Continuous Section Field segment** is defined by

$$
\mathcal{C} = \left( I,\, \lbrace P_{i}^{0}, P_{i}^{1}, w_{i}, \hat{w}_{i} \rbrace_{i=1}^{m} \right),
$$

where each component $i$ is associated with:

- an initial polygon $P_{i}^{0}$;
- a final polygon $P_{i}^{1}$;
- a scalar field $w_{i}:I\rightarrow\mathbb{R}$;
- a scalar field $\hat{w}_{i}:I\rightarrow\mathbb{R}$.

The polygons $P_{i}^{0}$ and $P_{i}^{1}$ are assumed to be homologous, i.e. they have the same number of ordered vertices:

$$
P_{i}^{0} = \mathrm{poly}(v_{i1}^{0},\ldots,v_{in_{i}}^{0}), \qquad P_{i}^{1} = \mathrm{poly}(v_{i1}^{1},\ldots,v_{in_{i}}^{1}),
$$

with corresponding vertices

$$
v_{ij}^{0} \leftrightarrow v_{ij}^{1}, \qquad v_{ij}^{0}, v_{ij}^{1} \in \mathbb{R}^{2}.
$$

For any $z\in I$, define

$$
\lambda(z)=\frac{z}{L}.
$$

The interpolated vertices are

$$
v_{ij}(z) = (1-\lambda(z))\,v_{ij}^{0} + \lambda(z)\,v_{ij}^{1}.
$$

The corresponding polygon at station $z$ is

$$
P_{i}(z) = \mathrm{poly}\!\left( v_{i1}(z),\ldots,v_{in_{i}}(z) \right).
$$

The Continuous Section Field is the map

$$
\mathcal{S}:I\rightarrow\mathcal{X},
$$

defined as

$$
\boxed{\mathcal{S}(z) = \left\lbrace \left( P_{i}(z),\, w_{i}(z),\, \hat{w}_{i}(z) \right) \right\rbrace_{i=1}^{m}}
$$

where $\mathcal{X}$ denotes the space of polygonal cross-sections whose regions carry two associated scalar fields.

In compact form:

$$
\boxed{\mathcal{S}(z) = \left\lbrace \left( \mathrm{poly}\!\left( (1-\lambda(z))\,v_{ij}^{0}+\lambda(z)\,v_{ij}^{1} \right)_{j=1}^{n_{i}},\, w_{i}(z),\, \hat{w}_{i}(z) \right) \right\rbrace_{i=1}^{m}}
$$

## Interpretation

At each axial station $z$, the field returns a polygonal cross-section composed of $m$ regions.  
Each region has:

1. its interpolated polygonal geometry $P_{i}(z)$;
2. a scalar field $w_{i}(z)$;
3. a second scalar field $\hat{w}_{i}(z)$.

The sectional quantities used later in the analysis are obtained by applying suitable geometric or mechanical functionals to $\mathcal{S}(z)$ at arbitrary stations along the member axis.

## Piecewise stacking

For stacked segments, let

$$
0=z_{0}<z_{1}<\cdots<z_{N}=L
$$

be a partition of the member axis.  
A local field

$$
\mathcal{S}_{k}:[z_{k},z_{k+1}]\rightarrow\mathcal{X}
$$

is defined on each interval.  
The global field is then

$$
\mathcal{S}(z)=\mathcal{S}_{k}(z) \qquad \text{for } z\in[z_{k},z_{k+1}].
$$

This defines a piecewise continuous section field.

## Scope

The formulation is restricted to straight-axis section fields generated by paired polygonal cross-sections.  
It is not a general solid-modeling formulation and does not describe arbitrary three-dimensional geometries, curved-axis members, or general ruled surfaces.
