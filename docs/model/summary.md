...
A second verification example is introduced to isolate the construction of the continuous section field from the structural response analysis considered in the NREL tower case. The purpose is to provide a controlled verification case in which geometric variation and participation-field variation are separated while a closed-form reference remains available. The compensation mechanism creates a non-trivial benchmark in which one sectional quantity remains invariant while others do not.. 
The external beam solver is intentionally removed, and the member is reduced to a stacked rectangular section for which the weighted area, centroid coordinate, and bending inertias can be evaluated independently in closed form. The compensation is introduced intentionally to create a non-trivial verification case in which one sectional quantity remains invariant while others do not. This allows the verification to test the independence of sectional-property evolution rather than merely detecting monotonic geometric changes.
...
<p align="center">
  <em>Figure 8. Shear/torsion participation fields in the second interval.</em>
</p>
