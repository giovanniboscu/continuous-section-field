...
You define the section as a set of polygons at two end stations. CSF interpolates the vertices linearly between them, producing a smooth tapered geometry at any $z$.
 ...
 Each pairs of corrisponding polygons get its own weight law $w_i(z)$ for axial and bending properties, and an independent shear weight law $\kappa_i(z)$ for shear properties - either an analytical expression (polynomial, exponential) or a lookup table of discrete values.
