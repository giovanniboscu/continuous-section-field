...
`cover_outer`, `cover_inner` $\rightarrow r_\mathrm{cover}(\psi)$

`core_inner`, `pcbar_host_layer` $\rightarrow r_\mathrm{core}(\psi)$

`pcbar_00` $\ldots$ `pcbar_31` $\rightarrow r_\mathrm{bar,A}(\psi)$
