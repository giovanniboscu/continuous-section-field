...
The illustrative intermediate values are obtained from the normalized time parameter:
...
| year | tau | psi(t) | corrosion degree |
| ---: | --: | -----: | ----------------: |
| 0 | 0.000 | 0.0000 | 0.00% |
...
The fitted cover-concrete reduction factor is:

$$r_{\mathrm{cover}}(\psi)=0.25\tan^{-1}\left(\frac{0.149-\psi}{0.025}\right)+0.65$$

The fitted core-concrete reduction factor is:
...
| year | r_cover | r_core | r_bar,A | r_bar,fy |
| ---: | ------: | -----: | ------: | -------: |
...
For the CSF model, the degradation is applied radially as follows.
...
where $f(z/L)$ is a prescribed CSF exposure field. The uniform case is recovered with:

$$f(z/L)=1$$

For non-uniform axial exposure, different prescribed fields can be used, for example localized, plateau, lookup-table, or multi-zone exposure laws. In all cases, the PDF-derived degradation laws remain functions of $\psi$, while CSF supplies the axial field through $\psi(z,t)$.
