...
The full 2D beam domain is therefore:

`Omega = { (x, y) : x in [0, l], y in A(x) }`

This means that the geometric/material model does not describe only a set of scalar functions, but a reduced 2D domain generated by those functions.

At this stage, the domain is completely identified by:

- the longitudinal interval `x in [0, l]`;
- the prescribed geometric functions `c(x)` and `h(x)`;
- the derived boundaries `h_l(x)` and `h_u(x)`;
- the homogeneous isotropic linear-elastic material parameters `E` and `G`.
