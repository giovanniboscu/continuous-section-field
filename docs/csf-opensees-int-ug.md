...
It is a **CSF data file**: it contains *station-by-station section properties* plus the **centroid offsets** needed by the builder to reconstruct the centroid axis.
...
## 1) Header block (human hints + parser hints)
...
If CSF used Gauss–Lobatto stationing, then:
- \(z_0 = 0\),
- \(z_{N-1} = L\),
...
## 3) Informational nodes (not used for precision)
...
- they may encode a best-fit line through centroid offsets,
- they are kept for human readability or legacy scripts.

A correct builder **must not** use them to reconstruct the station geometry.  
Instead it must use:
- `CSF_Z_STATIONS` for positions, and
- per-station `xc yc` for the centroid axis.
...
- The builder usually reuses this orientation to keep bending directions consistent.
...
- `<E>`, `<G>`  
  ...
  - **reference moduli** \(E_\text{ref}, G_\text{ref}\) (with modular properties).
...
- `<J>`  
  Torsional constant used by OpenSees (torsional stiffness \(GJ\)).  
...
- `<xc> <yc>` (**CSF-only extension fields**)  
  ...
  They are intended for the builder to place centroid-axis nodes.
...
The member is described by the list \(\{z_i, A_i, I_{z,i}, I_{y,i}, J_i, x_{c,i}, y_{c,i}\}\).
...
- A robust CSF builder typically **does not** rely on these lines, because CSF needs an explicit mapping from:
...
## 7) What the builder must do with this file (minimal contract)

A correct CSF → OpenSees builder must:

1) Read `CSF_Z_STATIONS`  
   - enforce \(N = \#\text{stations} = \#\text{section lines}\).

2) Read each `section Elastic ...` line  
   - extract \(E, A, I_z, I_y, G, J\) **and** the extra fields `xc yc`.
...
4) Choose the integration mapping:
