...
def compute_saint_venant_J_2remove(
    section: Any,
    grid_h: Optional[float] = None,
    *,
    auto_n: int = 80,
    pad: float = 0.0,
    ...
    """
    Weighted Saint-Venant torsional constant for a Section-like object
    containing multiple polygonal solids.

    Parameters
    ----------
    section:
        Section-like object with .polygons iterable.
    grid_h:
        Uniform grid spacing. If None, a per-polygon automatic spacing is used.
    auto_n:
        Used only if grid_h is None. Controls automatic h via bbox spans.
    pad:
        Optional padding added to each polygon bounding box before meshing.
    eps_l:
        Linear tolerance used in point-on-edge detection. If None, uses EPS_L.
    max_iter:
        SOR iteration cap per polygon.
    tol:
        SOR convergence threshold on max update in ψ.
    omega:
        SOR relaxation factor; omega=1.0 is Gauss-Seidel.
    max_grid_nodes:
        Safety cap on nx*ny to prevent extremely slow runs.

    Returns
    -------
    float
        J_total = Σ (weight_i * J_i). No sign normalization is applied.
    """
    if eps_l is None:
        eps_l = float(EPS_L)
    ...
    for poly in polys:
        if not hasattr(poly, "weight"):
            raise ValueError(f"Polygon '{getattr(poly, 'name', None)}' has no .weight attribute.")
        w = float(getattr(poly, "weight"))

        if w == 0.0:
            continue

        verts = _poly_vertices_xy(poly)
        if len(verts) < 3:
            raise ValueError(f"Polygon '{getattr(poly, 'name', None)}' has <3 vertices.")

        h = float(grid_h) if grid_h is not None else _auto_grid_h_from_bbox(verts, auto_n)

        J_i = _compute_J_solid_polygon_grid(
            verts,
            grid_h=h,
            pad=pad,
            eps_l=float(eps_l),
            ...
            tol=tol,
            omega=omega,
            max_grid_nodes=int(max_grid_nodes),
        )
...
def export_opensees_discretized_sections2remove(field: ContinuousSectionField, z_coords: list, E_val: float, filename: str = "opensees_sections.tcl"):
    """
...
def integrate_volume_wait(field: ContinuousSectionField, n_points: int =20) -> float:
    ...
    ----------
    field
        ContinuousSectionField instance. Must provide:
          - field.z0, field.z1 (float): endpoint coordinates along the axis
          - field.section(z: float): returns a section object evaluable by section_properties
    n_points
        Number of Gauss–Legendre integration points (>= 1).
        Typical choices:
          - 2 to 4: fast, coarse
          - 5 to 8: good default for smooth variation
          - 10+   : for strongly varying / piecewise laws

    Returns
    -------
    float
        Approximate volume V = ∫ A(z) dz

    Notes
    -----
    1) Quadrature domain mapping
       Gauss–Legendre nodes/weights are defined on [-1, 1]. We map them to [z0, z1] via:

           z(xi) = z_mid + (L/2) * xi
           dz    = (L/2) dxi

       where z_mid = (z0 + z1)/2 and L = |z1 - z0|.

    2) What "A" means
       This function integrates the area returned by section_properties(section)["A"].
       If your project distinguishes between:
         - geometric area (pure geometry),
         - transformed area (including weights/material ratios),
       decide which one you want to integrate and change the key accordingly
       (e.g., "A_tr").

    3) Discontinuities / step laws
       If w(z) is piecewise (e.g., step activation), a single high-order Gauss rule may
       not be optimal. In that case, consider splitting the integral into sub-intervals
       aligned with discontinuity points and summing integrate_volume over each interval.
    """
    if not isinstance(n_points, int) or n_points < 1:
        raise ValueError("n_points must be an integer >= 1")
    ...
    # Degenerate case: zero-length element -> zero volume
    if L == 0.0:
        return 0.0
    ...
    # Gauss–Legendre nodes (xi) and weights (wi) on [-1, 1].
    # numpy provides stable, high-quality values for arbitrary n.
    xi, wi = np.polynomial.legendre.leggauss(n_points)

    volume = 0.0

    # Evaluate the integrand at Gauss points
    for x, w in zip(xi, wi):
        # Map node x in [-1,1] to physical coordinate z in [z0,z1]
        z = z_mid + half_L * float(x)

        # Evaluate section at z and extract area
        sec = field.section(z)
        props = section_properties(sec)

        # --- IMPORTANT: choose the correct area key for your model ---
        # Default: use "A" (net section area returned by your section_properties).
        A = float(props["A"])

        # Accumulate: V ≈ Σ A(z_i) * w_i * (L/2)
        volume += A * float(w) * half_L
...
def gauss_rule(n_points: int) -> Tuple[List[float], List[float]]:
    ...
    This is useful for debugging, documentation, or if you want to precompute rules
    for repeated integrations.

    Returns
    -------
    (nodes, weights) as Python lists of floats
    """
    if not isinstance(n_points, int) or n_points < 1:
        raise ValueError("n_points must be an integer >= 1")

    xi, wi = np.polynomial.legendre.leggauss(n_points)
    return [float(x) for x in xi], [float(w) for w in wi]
    # end gauss_rule
...
def integrate_volume_5points(field: ContinuousSectionField) -> float:
    ...
    for xi, weight in gauss_points:
        ...
        z_phys = ((field.z1 - field.z0) * xi + (field.z1 + field.z0)) / 2.0# a absolute
        # Get sectional area at the Gauss point
        section = field.section(z_phys)
        A = section_properties(section)["A"] # absolute 
...
def section_full_analysis(section: Section,alpha: float = 0.0):
    """
...
class ContinuousSectionField:
    ...
    def section_area_by_weight(
        ...
        # --- 1) Interpolated section at z (polygons may carry RELATIVE weights here).
        sec = self.section(z)

        # --- Robust polygon area (shoelace), works for open or closed vertex lists.
        def _poly_vertices(p) -> List[Point]:
            verts = getattr(p, "vertices", None)
            if verts is None:
                raise ValueError(f"Polygon '{getattr(p,'name',None)}' has no .vertices attribute.")
            out: List[Point] = []
            for v in verts:
                if isinstance(v, (list, tuple)) and len(v) >= 2:
                    x, y = float(v[0]), float(v[1])
                else:
                    x, y = float(getattr(v, "x")), float(getattr(v, "y"))
                out.append((x, y))
            return out

        def _poly_area_abs(verts: List[Point]) -> float:
            if len(verts) < 3:
                return 0.0
            if verts[0] == verts[-1]:
                verts = verts[:-1]
            if len(verts) < 3:
                return 0.0
            s = 0.0
            for (x1, y1), (x2, y2) in zip(verts, verts[1:] + [verts[0]]):
                s += x1 * y2 - y1 * x2
            return abs(0.5 * s)

        # --- 2) Build stable container map on S0 (by polygon name).
        s0_polys = self.s0.polygons
        if not s0_polys:
            raise ValueError("S0 has no polygons; cannot build a stable container map.")
        ...
        for p in s0_polys:
            nm = getattr(p, "name", None)
            if not nm:
                raise ValueError("All S0 polygons must have a non-empty unique name.")
            if nm in seen:
                raise ValueError(f"Duplicate polygon name at S0: '{nm}'.")
            seen.add(nm)
            s0_names.append(nm)

        # Map: child_name -> parent_name (or None)
        container_of: Dict[str, Optional[str]] = {}
        for i, p0 in enumerate(s0_polys):
            child_name = s0_names[i]
            parent_idx = self.get_container_polygon_index(p0, i)

            # Forbid self-container.
            if parent_idx == i:
                parent_idx = None

            if parent_idx is None:
                container_of[child_name] = None
            else:
                if not isinstance(parent_idx, int) or not (0 <= parent_idx < len(s0_polys)):
                    raise ValueError(
                        f"get_container_polygon_index() returned invalid index {parent_idx} for '{child_name}'."
                    )
                parent_name = s0_names[parent_idx]
                container_of[child_name] = None if parent_name == child_name else parent_name

        # --- 3) Map current section polygons by name (w_rel + area).
        sec_by_name: Dict[str, Dict[str, Any]] = {}
        for idx, p in enumerate(sec.polygons):
            nm = getattr(p, "name", None)
            if not nm:
                raise ValueError("All section polygons must have a non-empty name.")
            if nm in sec_by_name:
                raise ValueError(f"Duplicate polygon name at section(z): '{nm}'.")
            w_rel = float(getattr(p, "weight"))
            area = _poly_area_abs(_poly_vertices(p))
            sec_by_name[nm] = {"idx": idx, "poly": p, "w_rel": w_rel, "area_abs": float(area)}

        # Ensure S0 names exist at z.
        for nm in container_of.keys():
            if nm not in sec_by_name:
                raise ValueError(f"Polygon '{nm}' exists in S0 but not in section(z).")

        # --- 4) Reconstruct ABSOLUTE weights via container chain.
        w_abs_cache: Dict[str, float] = {}

        def _w_abs(nm: str) -> float:
            if nm in w_abs_cache:
                return w_abs_cache[nm]
            parent_nm = container_of.get(nm)
            w_rel = sec_by_name[nm]["w_rel"]
            if parent_nm is None:
                w_abs_cache[nm] = w_rel
            else:
                w_abs_cache[nm] = w_rel + _w_abs(parent_nm)
            return w_abs_cache[nm]

        for nm in container_of.keys():
            _w_abs(nm)

        # --- 5) Group RAW polygon areas by reconstructed ABSOLUTE weight.
        def _bin_w(w: float) -> float:
            if w_tol and w_tol > 0.0:
                return round(w / w_tol) * w_tol
            return w

        groups: Dict[float, Dict[str, Any]] = {}
        per_poly: List[Dict[str, Any]] = []

        for nm in container_of.keys():
            w_abs = _bin_w(w_abs_cache[nm])
            a = float(sec_by_name[nm]["area_abs"])

            groups.setdefault(w_abs, {"w": w_abs, "area": 0.0, "polygons": []})
            groups[w_abs]["area"] += a
            groups[w_abs]["polygons"].append(nm)

            per_poly.append(
                {
                    "idx": sec_by_name[nm]["idx"],
                    "name": nm,
                    "container": container_of[nm],
                    "w_rel": sec_by_name[nm]["w_rel"],
                    "w_abs": w_abs,
                    "area_abs": a,
                }
        ...
        # total_area = sum of areas for NON-zero absolute weights (overlay rule)
        total_area = 0.0
        for w, g in groups.items():
            if abs(float(w)) > float(zero_w_eps):
                total_area += max(0.0, float(g["area"]))

        groups_list = sorted(groups.values(), key=lambda d: d["w"])
        for g in groups_list:
            g["area_fraction"] = (g["area"] / total_area) if total_area > 0.0 else 0.0

        if debug:
            print("=== section_area_by_weight debug (v5 raw areas, abs weights) ===")
            print(f"z = {z}  w_tol={w_tol}  zero_w_eps={zero_w_eps}")
            for r in sorted(per_poly, key=lambda rr: rr["idx"]):
                print(
                    f"  idx={r['idx']:>3d}  {r['name']:<16s}  "
                    f"parent={str(r['container']):<12s}  "
                    f"w_rel={r['w_rel']:+.6g}  w_abs={r['w_abs']:+.6g}  A={r['area_abs']:.6g}"
                )
            print("=== groups ===")
            for g in groups_list:
                print(f"  W_abs={g['w']:+.6g}  A={g['area']:.6g}  polys={g['polygons']}")
            print(f"total_area (non-zero W_abs) = {total_area:.6g}")
            print("=== end debug ===")
    ...
    def section_area_by_weight(
        ...
        total_area_all = sum(max(0.0, g["area"]) for g in groups.values())

        # se vuoi mantenere anche il totale "materiale" (W!=0)
        total_area_nonzero = sum(max(0.0, g["area"]) for w, g in groups.items()
                                 if abs(float(w)) > float(zero_w_eps))

        total_area = total_area_nonzero  # oppure total_area_all, dipende da cosa vuoi stampare

        for g in groups_list:
            g["area_fraction"] = (g["area"] / total_area_all) if total_area_all > 0.0 else 0.0
    ...
    def section_area_by_weight2(
        ...
        """
        Compute an area breakdown at section z grouped by ABSOLUTE weight (material W).

        Invariants enforced (to avoid negative exclusive-area artifacts):
        - A polygon can NOT be its own container.
        - Container chains can NOT have cycles.
        - Children lists are de-duplicated and can NOT contain the parent itself.

        Area model:
        - We work with absolute geometric areas: A_abs(p) >= 0 (orientation-independent)
        - Exclusive area partitions the union of polygons:
            A_excl(p) = A_abs(p) - sum(A_abs(child) for child in immediate_children(p))

        Absolute weights:
        - Polygons in `sec` carry RELATIVE weights w_rel at z.
        - Absolute weights are reconstructed via container chain:
            w_abs(p) = w_rel(p) + w_abs(container(p))  if container exists
            w_abs(p) = w_rel(p)                        if outermost
        """
        # --- 1) Interpolated section at z (polygons carry RELATIVE weights here).
        sec = self.section(z)

        # ------------------------------------------------------------
        # Robust polygon area (shoelace), independent of polygon_area_centroid().
        # Works for open or closed vertex lists.
        # ------------------------------------------------------------
        def _poly_vertices(p) -> List[Point]:
            verts = getattr(p, "vertices", None)
            if verts is None:
            ...
            out: List[Point] = []
            for v in verts:
                # Support either [x, y] lists/tuples or small point objects (x,y).
                if isinstance(v, (list, tuple)) and len(v) >= 2:
                    x, y = float(v[0]), float(v[1])
                else:
                    x, y = float(getattr(v, "x")), float(getattr(v, "y"))
        ...
        def _poly_area_signed(verts: List[Point]) -> float:
            if len(verts) < 3:
                return 0.0
            # If closed ring (last equals first), drop last to avoid double edge.
            if verts[0] == verts[-1]:
                verts = verts[:-1]
            n = len(verts)
            if n < 3:
        ...
        def _poly_area_abs(p) -> float:
            verts = _poly_vertices(p)
            return abs(_poly_area_signed(verts))

        # --- 2) Build a stable container map on S0 (by polygon name).
        s0_polys = self.s0.polygons
        if not s0_polys:
            raise ValueError("S0 has no polygons; cannot build a stable container map.")

        # Validate and collect unique names.
        s0_names: List[str] = []
        seen = set()
        for p in s0_polys:
            nm = getattr(p, "name", None)
            if not nm:
                raise ValueError("All S0 polygons must have a non-empty unique name.")
            if nm in seen:
                raise ValueError(f"Duplicate polygon name at S0: '{nm}'. Names must be unique.")
            seen.add(nm)
            s0_names.append(nm)

        # Map: poly_name -> container_name (or None)
        container_of: Dict[str, Optional[str]] = {}
        ...
        for i, p0 in enumerate(s0_polys):
            ...
            # --- Critical safety: forbid self-container (common root of negative A_excl).
            if parent_idx == i:
                parent_idx = None
            ...
            if not isinstance(parent_idx, int) or parent_idx < 0 or parent_idx >= len(s0_polys):
                raise ValueError(
                    f"get_container_polygon_index() returned invalid index {parent_idx} "
                    f"for polygon '{child_name}'."
                )

            parent_name = s0_names[parent_idx]

            # Defensive: also forbid name-level self-container.
            if parent_name == child_name:
                container_of[child_name] = None
            else:
                container_of[child_name] = parent_name

        # --- 2b) Validate container chains: no cycles (Floyd cycle detection).
        for nm in s0_names:
            slow = nm
            fast = nm
            while True:
                slow = container_of.get(slow)
                fast = container_of.get(fast)
                if fast is None:
                    break
                fast = container_of.get(fast)
                if slow is None or fast is None:
                    break
                if slow == fast:
                    raise ValueError(f"Container cycle detected involving '{nm}'. Check nesting logic.")

        # --- 2c) Build immediate children map: parent_name -> [child_names...]
        ...
        # De-duplicate children lists while preserving order.
        for parent_name, kids in list(children_of.items()):
            seen_k = set()
            uniq: List[str] = []
            for k in kids:
                if k not in seen_k:
                    uniq.append(k)
                    seen_k.add(k)
            children_of[parent_name] = uniq

        # --- 3) Map current section polygons by name (w_rel + A_abs at z).
        sec_by_name: Dict[str, Dict[str, Any]] = {}
        for idx, p in enumerate(sec.polygons):
            nm = getattr(p, "name", None)
            if not nm:
                raise ValueError("All section polygons must have a name (propagated from S0).")
            if nm in sec_by_name:
                raise ValueError(f"Duplicate polygon name at section(z): '{nm}'. Names must be unique.")
            sec_by_name[nm] = {
                "idx": idx,
                "poly": p,
                "w_rel": float(getattr(p, "weight")),
                "area_abs": _poly_area_abs(p),
            }

        # Ensure all S0 names exist at z (consistency check).
        for nm in container_of.keys():
            if nm not in sec_by_name:
                raise ValueError(
                    f"Polygon name mismatch: '{nm}' exists in S0 but not in section(z). "
                    "Ensure S0/S1 have consistent polygon names."
                )

        # --- 4) Reconstruct ABSOLUTE weights using container chain.
        w_abs_cache: Dict[str, float] = {}
        ...
        # --- 5) Exclusive areas.
        # IMPORTANT: Do NOT raise if slightly negative due to numeric noise.
        # If meaningfully negative, nesting map is wrong; raise with clear diagnostics.
        eps = 1e-12
        area_excl: Dict[str, float] = {}
        for nm in container_of.keys():
            a0 = float(sec_by_name[nm]["area_abs"])
            a_children = 0.0
            for ch in children_of.get(nm, []):
                if ch == nm:
                    continue
                a_children += float(sec_by_name[ch]["area_abs"])
            a_ex = a0 - a_children

            if a_ex < 0.0 and abs(a_ex) <= eps:
                a_ex = 0.0
            if a_ex < -eps:
                raise ValueError(
                    f"Exclusive area became negative for '{nm}': "
                    f"A_abs={a0:.12g}, sum(children)={a_children:.12g}, A_excl={a_ex:.12g}. "
                    "This indicates an invalid container/children relationship (or an area computation bug)."
                )

            area_excl[nm] = a_ex

        # --- 6) Group exclusive areas by ABSOLUTE weight.
        def _bin_w(w: float) -> float:
            if w_tol and w_tol > 0.0:
                return round(w / w_tol) * w_tol
            return w
        ...
        groups: Dict[float, Dict[str, Any]] = {}
        for nm in container_of.keys():
            w = _bin_w(w_abs_cache[nm])
            groups.setdefault(w, {"w": w, "area": 0.0, "polygons": []})
            groups[w]["area"] += area_excl[nm]
            groups[w]["polygons"].append(nm)

        # Keep the original "net-area" style if you want to exclude negative areas:
        # (Here areas are non-negative by construction, so this is equivalent to sum(area).)
        total_area = sum(max(0.0, g["area"]) for g in groups.values())
        ...
        if debug:
            print("=== section_area_by_weight debug (v3) ===")
            print(f"z = {z}")
            print("\n-- polygons (S0 order) --")
            for nm in s0_names:
                parent = container_of[nm]
                print(
                    f"  {nm:>16s}  container={str(parent):>16s}  "
                    f"w_rel={sec_by_name[nm]['w_rel']:+.6g}  w_abs={w_abs_cache[nm]:+.6g}  "
                    f"A_abs={sec_by_name[nm]['area_abs']:.6g}  A_excl={area_excl[nm]:.6g}"
                )
            print("\n-- children --")
            for parent, kids in children_of.items():
                print(f"  {str(parent):>16s} -> {kids}")
            print("\n-- groups --")
            for g in groups_list:
                print(f"  W={g['w']:+.6g}  A={g['area']:.6g}  polys={g['polygons']}")
            print("=== end debug ===")
        ...
        if include_per_polygon:
            per_poly = []
            for nm in sorted(container_of.keys(), key=lambda n: sec_by_name[n]["idx"]):
                per_poly.append(
                    {
                        "idx": sec_by_name[nm]["idx"],
                        "name": nm,
                        "container": container_of[nm],
                        "w_rel": sec_by_name[nm]["w_rel"],
                        "w_abs": w_abs_cache[nm],
                        "area_abs": sec_by_name[nm]["area_abs"],
                        "area_excl": area_excl[nm],
                    }
                )
            out["per_polygon"] = per_poly
...
class Visualizer:
    ...
    def plot_properties(self, keys_to_plot=None,alpha=1,num_points=100):
        ...
        #available_keys = ["A", "Cx", "Cy", "Ix", "Iy", "Ixy", "J","I1"]
        #available_keys = ["A", "Cx", "Cy", "Ix", "Iy", "Ixy", "J","I1"]
        '''
        available_keys1= [
            'Cy',
        ...
        if len(keys_to_plot) > 0:
            ...
            for z in z_values:
                ...
                current_section = self.field.section(z) # a
                
                # CALL: Use the global function section_properties and pass the section
                props = section_full_analysis(current_section,alpha=alpha) #alpha not used
                for key in keys_to_plot:
                    if key in props:
                        value = props[key]

                        if abs(value) < EPS_L:
                            props[key] = 0.0  # below precision
    ...
    """
    plot_section_2d (drop-in replacement)

    This file contains a revised version of `plot_section_2d()` that fixes a subtle but important
    index-space bug in the legend "container=#..." computation.

    Key fix
    -------
    `get_container_polygon_index()` is documented to return an index in `self.field.s0.polygons`.
    Therefore, the lookup must be performed in S0-space (by name), then mapped back to the
    current `sec.polygons` order (also by name) for legend display.

    This avoids mixing:
    - `idx` from `sec.polygons` (section evaluated at z)
    with:
    - indices returned in `self.field.s0.polygons` (S0 topology space)

    Secondary fix
    -------------
    Avoid double-closing polygons when the vertex list is already explicitly closed
    (last vertex equals first vertex).

    How to use
    ----------
    Copy/paste the function below into your class (replacing the existing method).
    """
