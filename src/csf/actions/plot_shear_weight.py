...
# -----------------------------------------------------------------------------
# IMPORTANT:
# - Keep this aligned with the baseline CSFActions ActionSpec for plot_shear_weight.
...
def _run(
    ...
    *,
    ...
    def _noop_show(*args: Any, **kwargs: Any) -> None:
        """Temporary replacement for plt.show() during plot_shear_weight.

        Visualizer.plot_shear_weight currently ends with plt.show() which would
    ...
    if file_outputs:
        ...
        if not figs:
            raise RuntimeError("No figures were generated for plot_shear_weight file output.")
