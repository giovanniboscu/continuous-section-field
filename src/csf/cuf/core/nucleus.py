...
class StrainContribution:
    ...
    transverse_derivative:
        Basis-factor selector. ``None`` uses ``F`` directly; ``"y"`` and
        ``"z"`` use the transverse derivatives; ``"x"`` uses the optional
        longitudinal basis derivative ``dF/dx``.  The historical attribute
...
class FundamentalNucleusProvider:
    ...
    # strain index m or n.
    _STRAIN_KINEMATICS = {
        # Classical CUF terms for F = F(y,z).
        # epsilon_xx = F * u_x,x
        1: (
    ...
    # Extra product-rule terms when F = F(x,y,z):
    #
    #   d(F*u)/dx = F,x*u + F*u,x.
    #
    # They are kept separate so a legacy basis with dF/dx = 0 uses exactly
    # the historical nucleus structure and pays no additional assembly cost.
    _LONGITUDINAL_BASIS_KINEMATICS = {
        # epsilon_xx += F,x * u_x
        1: (
            StrainContribution(0, "x", 0),
        ),
        2: tuple(),
        3: tuple(),
        4: tuple(),
        # gamma_xz += F,x * u_z
        5: (
            StrainContribution(2, "x", 0),
        ),
        # gamma_xy += F,x * u_y
        6: (
            StrainContribution(1, "x", 0),
        ),
    }
    ...
    def __init__(self, sectional_coefficients: Any) -> None:
        ...
        self.longitudinal_basis_derivative_enabled = bool(
            getattr(
                sectional_coefficients,
                "longitudinal_basis_derivative_enabled",
                False,
            )
        )
    ...
    def strain_kinematics(
        ...
        voigt_index: int,
        *,
        include_longitudinal_basis_derivative: bool = False,
    ) -> Tuple[StrainContribution, ...]:
        """Return the kinematic contributions of one Voigt strain.

        ``include_longitudinal_basis_derivative`` adds the product-rule terms
        generated by an explicit ``F(x,y,z)`` dependence.  Its default keeps
        the historical public behavior unchanged.
        """

        if voigt_index not in cls._STRAIN_KINEMATICS:
            raise IndexError("voigt_index must be in 1..6")

        contributions = cls._STRAIN_KINEMATICS[voigt_index]
        if include_longitudinal_basis_derivative:
            contributions += cls._LONGITUDINAL_BASIS_KINEMATICS[
                voigt_index
            ]
    ...
    def K_block_structure(
        ...
        *,
        ...
        for m in range(1, 7):
            for test_kinematic in self.strain_kinematics(
                m,
                include_longitudinal_basis_derivative=(
                    self.longitudinal_basis_derivative_enabled
                ),
            ):
                ...
                for n in range(1, 7):
                    for trial_kinematic in self.strain_kinematics(
                        n,
                        include_longitudinal_basis_derivative=(
                            self.longitudinal_basis_derivative_enabled
                        ),
