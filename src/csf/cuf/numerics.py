# OPT-04 VECTORIZED TRANSVERSE BASIS
...
class FixedGaussPolygonIntegrator(SectionIntegrator):
    """Fixed Gauss integration over arbitrary simple polygonal CSF domains."""
    ...
    def _y_subintervals(domain):
        values = sorted(set(float(point[0]) for point in domain.vertices))
        if len(values) < 2:
    ...
    def quadrature_points(self, domain):
        ...
        for y0, y1 in self._y_subintervals(domain):
            ...
            for xi_y, w_y in zip(self.points, self.weights):
                ...
                for z0, z1 in self._z_intervals_at_y(
                    domain.vertices,
                    y,
                ):
                    z_mid = 0.5 * (z0 + z1)
    ...
    def integrate(self, domain, integrand):
        ...
        for y0, y1 in self._y_subintervals(domain):
            ...
            for xi_y, w_y in zip(self.points, self.weights):
                y = y_mid + y_jac * float(xi_y)
                for z0, z1 in self._z_intervals_at_y(domain.vertices, y):
